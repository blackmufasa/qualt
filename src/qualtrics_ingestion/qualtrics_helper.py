...
def upload_survey_data_into_stage(asql, tdb) -> bool:
    ...
    try:
        ...
        logging.exception('Could not upload into table from s3 due to %s', e, exc_info=True)
        return False
...
def cleanup_date_cols(adf) -> pd.DataFrame:
    ...
    """
    if 'complete' in adf:
        adf = adf.astype({'complete':'string'})
    if 'completed' in adf:
        adf = adf.astype({'completed':'string'})
    for cols in list(adf):
        ###[amaheswa] True False values for complete and completed
        if cols in cfg.bool_cols:
            adf[cols]=adf[cols].map({'True': 'TRUE', 'False': 'FALSE'})
        ###
        adf[cols] = adf[cols].replace('None', '')
        if cols in cfg.numeric_cols_body:
            #adf[cols] = adf[cols].fillna(0)
            ###[amaheswa] validate numeric columns
            adf[cols] = adf[cols].replace(r'\D', pd.NA, regex = True).fillna('')
        else:
            adf[cols] = adf[cols].fillna('')
    for cols in cfg.body_date_cols:
        if cols in adf:
            ###[amaheswa] validate date columns
            adf[cols] = pd.to_datetime(adf[cols], errors='coerce')
            adf[cols]=adf[cols].astype(str)
...
def upload_a_file_into_s3(afile, upload_path, abucket):
    ...
    try:
        ...
        s3.meta.client.upload_file(Filename=afile, Bucket=abucket, Key=upload_path)
    except Exception as e:
        logging.info(f'Could not upload into s3 due to {e}')
...
def remove_html_tags(display_text):
    ...
    return re.sub(html_tags, "", display_text)#.replace('\n','\\')
